    ...
    return(
      <div className="results-container">
        ...
        {this.state.result ?
          <div className="results-list-container">
            <div className="result-info">
              <img className="result-photo" src={playlistPhoto ? playlistPhoto : result.photoUrl}></img>
              <div className="result-title">
                <h1>{result.title ? result.title : author}</h1>
              </div>
              <div className="result-creator">
                {result.artist?
                  <Link to={`artist/${result.artist_id}`}><h1>{creator}</h1></Link>
                    :
                <h1>{creator}</h1>}
              </div>
            </div>
        ...
        <div className="search-albums-container">
          ...
          <div className="search-albums-container">
            {this.props.album.length > 0? <AlbumsContainer albums={this.props.album}/> : ""}
          </div>
        </div> : ""}
      </div>
    );
